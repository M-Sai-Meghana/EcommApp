...
with mysql.connector.connect(host=host, user=user, password=password, db=db) as conn:
    ...
    cursor.execute("create table if not exists items(item_id varchar(8) primary key ,item_name char(30),qty int,status text,price int,category enum('Electronic Gadgets','Computers & Accessories','Clothing','Beauty & Toys','Groceries'))")
